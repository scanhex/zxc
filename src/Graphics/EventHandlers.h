...
void handleSkill(const Hero& hero, SkillNum skillNum, Scene3D& scene, Magnum::SceneGraph::DrawableGroup3D& group, Magnum::Timeline& timeline) {
	...
	auto shift = (Matrix4::rotationZ(Math::Rad{ (float)hero.getAngle() }) * Matrix4::translation({ 0.f, (float)dist, 0.f })).translation();
	obj->transform(Matrix4::translation(shift + Vector3{ (float)hero.getPosition().x_, (float)hero.getPosition().y_, 0.1f }));
